...
export class ControllerManager implements IControllerManager {
  ...
  constructor(private readonly _di: Di) {}
  ...
  public register<T = any>(
    ...
    pathOption =
      pathOption.length > 1 && pathOption.endsWith('/')
        ? pathOption.slice(0, -1)
        : pathOption
  ...
  public routes(
    ...
    if (
      this._routes.find((x) => {
        return x.key == key && x.path == path && x.method == method
      })
    )
  ...
  public getRoutes(): IControllerRoute[] {
    return this._routes.map((route) => {
      ...
      return {
        ...
        interceptors: [
          ...controller.options.interceptors,
          ...route.interceptors
        ]
      }
    })
