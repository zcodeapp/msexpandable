...
describe('Di Test', () => {
  ...
  it('Test get instance using providers', () => {
    const _di = Di.getInstance({
      cleanSingleton: true
    })

    _di.register(ExampleSimpleCallbackInject, {
      singleton: false,
      providers: [
        {
          class: ExampleSimpleCallback,
    ...
    const instance = _di.get(ExampleSimpleCallbackInject, {
      providers: [
        {
          class: ExampleSimpleCallback,
          factory: () => new ExampleSimpleCallback(() => 'content')
        }
      ]
    })
