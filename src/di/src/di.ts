...
export class Di implements IDi {
  ...
  static getInstance(_options?: IDiConstructor): IDi {
    if (_options?.cleanSingleton) this.instance = undefined

    if (!this.instance) {
      const _logger = _options?.logger ?? Logger.getInstance()
      this.instance = new Di(_logger, _options)
    } else if (_options) this.instance.updateOptions(_options)
   ...
   */
  public register<T>(
    key: TConstructor<T> | string,
    options?: IDiOptions
  ): void {
    try {
      this._logger.debug('Register instance', {
        key,
        options,
        diOptions: this._options
      })
      ...
      if (exists)
        this._instances[this._instances.findIndex((x) => x.unique == unique)] =
          payload
      else this._instances.push(payload)

      this._logger.debug('Success register key', { key, options })
    } catch (ex) {
      this._logger.fatal('Fatal error on try register instance', {
        key,
        options,
        ex
      })
  ...
  public provider<T = any>(key: TConstructor<T>, providers: any[]): void {
    try {
      ...
      this._logger.debug('Success add provider for instance', {
        key,
        providers
      })
    } catch (ex) {
  ...
  private _getInstanceProviders(instance: IDiInstance) {
    if (typeof instance.key == 'function') {
      ...
      const existingMetadata =
        Reflect.getMetadata('design:paramtypes', instance.key) || []
      if (existingMetadata && existingMetadata.length > 0) {
        this._logger.debug('Success get metadata from Reflect', {
          existingMetadata
        })
        const exclude: string[] = []
        return existingMetadata.map((metadata) =>
          this._parseMetadataValue(instance, metadata, exclude)
        )
      }
    }
   ...
   * @returns Dependency
   */
  private _parseMetadataValue(
    instance: IDiInstance,
    metadata: any,
    exclude: string[]
  ) {
  ...
  private _getKey<T>(key: TConstructor<T> | string): string {
    ...
    if (typeof key == 'string') _key = key.toLocaleLowerCase()

    if (typeof key == 'function') _key = key.name.toLocaleLowerCase()
  ...
  private _getInstance(
    instance: IDiInstance,
    providers?: any[],
    getProviders?: IDiGetOptionsProviders[]
  ) {
    ...
    if (typeof instance.key == 'function') {
      this._logger.debug('Return function value', {
        providers,
        instanceProviders: instance.providers
      })
      return new instance.key(
        ...this._providers(
          providers.length > 0 ? providers : instance.providers,
          getProviders
        )
      )
    }
  ...
  private _providers(
    ...
    return providers.map((provider) => {
      if (getProviders) {
        const _provider = getProviders.find((x) => {
          return (
        ...
        if (_provider) return _provider.factory()
      }
