...
export class Logger implements ILogger {
  ...
  public addPrefix(prefix: string): void {
    this._options = { ... this._options, ... { prefix: `${this._options.prefix ?? ""}${prefix}` }};
  }

  public debug(message: string, params?: any): void {
    if (this._options.level == ELoggerLevel.DEBUG)
      this._options.strategy.debug(`[DEBUG] ${this._options.prefix ?? ""}${message}`, JSON.stringify(params));
  }

  public info(message: string, params?: any): void {
    if (this._options.level <= ELoggerLevel.INFORMATION)
      this._options.strategy.info(`[INFO] ${this._options.prefix ?? ""}${message}`, JSON.stringify(params));
  }

  public warn(message: string, params?: any): void {
    if (this._options.level <= ELoggerLevel.WARNING)
      this._options.strategy.warn(`[WARN] ${this._options.prefix ?? ""}${message}`, JSON.stringify(params));
  }

  public error(message: string, params?: any): void {
    if (this._options.level <= ELoggerLevel.ERROR)
      this._options.strategy.error(`[ERROR] ${this._options.prefix ?? ""}${message}`, JSON.stringify(params));
  }

  public fatal(message: string, params?: any): void {
    if (this._options.level <= ELoggerLevel.FATAL)
      this._options.strategy.fatal(`[FATAL] ${this._options.prefix ?? ""}${message}`, JSON.stringify(params));
  }
}
